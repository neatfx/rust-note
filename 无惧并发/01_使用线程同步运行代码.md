...
Rust 一直很保守，只从线程中借用了 `v`，这意味着主线程能在理论上造成新建线程中的引用无效。通过告知 Rust 将 `v` 的所有权移动到新建线程，向 Rust 保证主线程不会再使用 `v`，这样新建线程得以安全运行。在这之后尝试在主线程中使用 `v` 时就会违反借用规则。关键字 `move` 覆盖了 Rust 默认保守的借用规定，不允许我们违反所有权规则。
